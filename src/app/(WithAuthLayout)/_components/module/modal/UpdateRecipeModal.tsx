...
const UpdateRecipeModal = ({ recipeId }: { recipeId: string }) => {
    ...
    const { data: singleRecipeData } = useGetSingleRecipe(recipeId);
    const { mutate: handleUpdateRecipe, isPending: UpdateRecipePending, isSuccess, data, } = useUpdateRecipe();

    const singleRecipe = singleRecipeData?.data || {};


    useEffect(() => {
        // Populate the form with fetched recipe data when available
        if (singleRecipe) {
            const defaultValues = {
                title: singleRecipe.title,
                description: singleRecipe.description,
                cookingTime: singleRecipe.cookingTime,
                isPremium: singleRecipe.isPremium,
                isPublished: singleRecipe.isPublished,
                ingredients: singleRecipe?.ingredients?.map((ingre: string) => ({ value: ingre })),
            };
            reset(defaultValues);
            setImagePreviews(singleRecipe.images || []); // Assuming images are URLs
        }
    }, [singleRecipe, reset]);
    ...
    const onSubmit: SubmitHandler<FieldValues> = (data) => {
        ...
        handleUpdateRecipe({ recipeId, recipeData: formData });
    };
