...
const EditableRecipeCardUser = ({ recipe }: { recipe: IRecipe }) => {
  ...
  return (
    <NextUiCard className="relative w-full max-w-2xl mx-auto border border-gray-200 shadow-sm rounded-lg overflow-hidden lg:flex lg:flex-row">
      ...
      {isPremium ? (
        <span className="inline-block absolute right-0 top-0 bg-yellow-500 text-white text-xs font-bold px-2 py-1 rounded-lg">
          Premium
        </span>
      ) : (
        <></>
        ...
        <CardFooter className="flex justify-between items-center pt-4">
          <div className="flex items-center space-x-4 ">
            <VoteButton
              ...
              upVote={upVoteCount}
            />
            <CommentButton count={totalComment} icon={<MessageSquare />} />
